...
var TestsAhoy = {
  ...
  _runOneTest: function(testObj, prop, onDone) {
    ...
    this.output("<br/>Running " + prop + "...");
    try {
      testObj[prop].call(testObj);
    } catch(e) {
      this.output("<br/>Test Failed: " + e);
    }
  ...
  proceed: function() {
    ...
    // So that we don't hang forever, set up a timer that will cancel the
    // async check if it takes too long.

    // Potential problem here is what if the timeout expires and then later
    // the async check really does resolve?  Then we'll have called
    // asyncCheckFinished() twice for the same check, which will result in
    // unpredictable behavior.  So maybe we need to give async checks some
    // kind of IDs so we know which one we're finishing and we don't try to
    // finish it twice.
    this.timer = window.setTimeout(function() {
                                    TestsAhoy.asyncCheckFinished(false);
                                  }, firstCheck.time);
    try {
